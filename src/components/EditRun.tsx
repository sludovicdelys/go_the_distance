...
import { getRun, updateRun, Run } from '../app/services/apiService';

const EditRun: React.FC<{ id: number }> = ({ id }) => {
  ...
  useEffect(() => {
    const fetchRunData = async () => {
      try {
        ...
        setFormData({
            ...runData,
            running_pace: runData.running_pace ? runData.running_pace.slice(0, 5) : '',
            start_time: runData.start_time ? runData.start_time.slice(0, 5) : '',
            time: runData.time ? runData.time.slice(0, 5) : '',
        });
      } catch (error) {
        setError('Error loading run data');
      }
    };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8 bg-gray-50">
      <section className="w-full max-w-2xl mx-auto px-4 sm:px-8">
        <div className="py-8">
          <h2 className="text-4xl font-bold text-center text-green-700 mb-10">Edit Run</h2>
          <div className="mb-8">
            <form className="space-y-4" onSubmit={handleSubmit} noValidate>
              ...
              <button type="submit" className="w-full bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700">Update</button>
